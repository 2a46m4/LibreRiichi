...
let busses: EventHandler<IncomingMessage>[] = Array(4)
  .fill(0)
  .map(() => new EventHandler())

let arena_busses = Array(4)
  .fill(0)
  .map((_, idx) => {
    let arena_bus: EventHandler<ServerEvent> = new EventHandler()
    busses[idx].register(
      keep_registered((data: IncomingMessage) => {
        if (data.message_type === MessageType.EVENT) {
          arena_bus.handle(data.data as ServerEvent)
        }
      }),
    )
...
let callback = (i: number, data: ServerEvent) => {
