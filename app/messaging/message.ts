...
function is_initial_message_action(
  data: any,
): data is import('./server_action_generated').InitialMessageAction {
  return is_object(data) && 'name' in data && is_string(data.name)
}

function is_join_arena_action(
  data: any,
): data is import('./server_action_generated').JoinArenaAction {
  return is_object(data) && 'arena_name' in data && is_string(data.arena_name)
}

function is_server_arena_action(
  data: any,
): data is import('./server_action_generated').ServerArenaAction {
  return (
    is_object(data) && 'arena_action' in data && data.arena_action !== undefined
  )
}

function is_list_arenas_action(
  data: any,
): data is import('./server_action_generated').ListArenasAction {
  return is_object(data)
}

function is_create_arena_action(
  data: any,
): data is import('./server_action_generated').CreateArenaAction {
  return is_object(data) && 'arena_name' in data && is_string(data.arena_name)
}

function is_arena_info_action(
  data: any,
): data is import('./server_action_generated').ArenaInfoAction {
  return is_object(data)
}

// ServerResponse validators
function is_generic_response(
  data: any,
): data is import('./server_response_generated').GenericResponse {
  return (
    is_object(data) &&
    'success' in data &&
...
function is_list_arenas_response(
  data: any,
): data is import('./server_response_generated').ListArenasResponse {
  return (
    is_object(data) &&
    'success' in data &&
    is_boolean(data.success) &&
...
function is_arena_info_response(
  data: any,
): data is import('./server_response_generated').ArenaInfoResponse {
  return (
    is_object(data) &&
    'success' in data &&
    is_boolean(data.success) &&
...
function is_server_arena_event(
  data: any,
): data is import('./server_event_generated').ServerArenaEvent {
  return (
    is_object(data) &&
    'arena_message' in data &&
    data.arena_message !== undefined
  )
}
...
function validate_request_data(data: any, errors: string[]): ValidationResult {
  ...
  const actionType =
    'serveraction_type' in data ? data.serveraction_type : undefined
...
function validate_response_data(data: any, errors: string[]): ValidationResult {
  ...
  const responseType =
    'serverresponse_type' in data ? data.serverresponse_type : undefined
...
function validate_event_data(data: any, errors: string[]): ValidationResult {
  ...
  const eventType =
    'serverevent_type' in data ? data.serverevent_type : undefined
